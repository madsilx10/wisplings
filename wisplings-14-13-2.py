...
async def run_use_mode():
    ...
    for i, acc in enumerate(selected, start=1):
        ...
        for j, sp in enumerate(spirit_targets):
            ...
            for item, quantity in tasks:
                await run_use_step(i, acc["token"], sp["id"], item["slug"], quantity)
                await asyncio.sleep(1)
            if j < len(spirit_targets) - 1:
                print(f"[{i}] jeda 20 detik sebelum spirit berikutnya...")
...
async def run_debug_mode():
    """Mode 11: print raw JSON dari collection-overview (semua spirit)
    + 1x care-use ke spirit pertama pake espresso, buat liat field % care."""
    tokens = load_tokens(TOKENS_FILE)
    if not tokens:
        print(f"{TOKENS_FILE} kosong. Jalankan mode Register dulu.")
        return
    ...
    print("=" * 60)
    print("[DEBUG] RAW collection-overview:")
    ...
    sp = spirits[0]
    spirit_id = sp["id"]
    print(f"\n[DEBUG] Spirit pertama: {sp.get('name')} (id {spirit_id})")

    # 2. Raw care-use 1x espresso
    await asyncio.sleep(2)
    print("\n" + "=" * 60)
    print("[DEBUG] RAW care-use 1x espresso:")
...
async def main():
    ...
    mode = input("Pilih mode [1-11]: ").strip()
