...
async def daily_status(token):
    url = f"{BASE_URL}/daily-status"
    headers = base_headers(f"daily-status:{uuid.uuid4()}", token)
    return await asyncio.to_thread(http_get, url, headers)
...
async def run_daily_claim_step(index, token, retries=3):
    for attempt in range(1, retries + 1):
        ...
        if status in (200, 201) and resp.get("ok"):
            ...
            chest_instance_id = bonus.get("chestInstanceId")
            print(f"[{index}] claim daily sukses. rewardCoins: {d.get('rewardCoins')}, "
                  f"rewardXp: {d.get('rewardXp')}, bonusCoins: {bonus.get('coins')}, "
                  f"balanceCoins: {d.get('balanceCoins')}, streakDay: {d.get('streakDay')}, "
                  f"consecutiveDays: {d.get('consecutiveDays')}"
                  + (f", chestInstanceId: {chest_instance_id}" if chest_instance_id else ""))
            d["_chestInstanceId"] = chest_instance_id
            return d
        if status == 500 and attempt < retries:
            wait = 2 * attempt
...
async def run_buy_mode():
    ...
    limit_input = input("Limit stok maksimum per item (kosongkan = gak ada limit): ").strip()
    stock_limit = int(limit_input) if limit_input.isdigit() else None

    selected = select_tokens(tokens)
    ringkasan = ", ".join(f"{it['name']} x{q}" for it, q in tasks)
    limit_info = f", limit stok {stock_limit}" if stock_limit else ""
    print(f"\nBeli [{ringkasan}] buat {len(selected)} akun{limit_info}...\n")

    for i, acc in enumerate(selected, start=1):
        print(f"[{i}] @{acc['username']} (id {acc['telegramId']})")
        for item, quantity in tasks:
            if stock_limit:
                # beli 1x dulu buat tau quantityOwned saat ini
                result1 = await run_buy_step(i, acc["token"], item["slug"], 1)
                await asyncio.sleep(1)
                if result1 is None:
                    continue
                owned = result1.get("quantityOwned", 0)
                if owned >= stock_limit:
                    print(f"[{i}] {item['name']}: stok udah {owned}, limit {stock_limit} — stop beli.")
                    continue
                # hitung sisa yang bisa dibeli
                remaining = min(quantity - 1, stock_limit - owned)
                if remaining > 0:
                    await run_buy_step(i, acc["token"], item["slug"], remaining)
                    await asyncio.sleep(1)
                else:
                    print(f"[{i}] {item['name']}: setelah 1x beli, stok {owned} udah di limit {stock_limit}.")
            else:
        ...
        for j, sp in enumerate(spirit_targets):
            ...
            for item, quantity in tasks:
                ...
                if need_field:
                    ...
                    if current_bps >= CAP_BPS:
                        print(f"[{i}] skip {item['name']} — {need_field} udah 100%, gak perlu.")
                        continue

                    # pake 1x dulu buat tau restoredBps aktual dari server
                    result = await run_use_step(i, acc["token"], sp["id"], item["slug"], 1)
                    if result is None:
                        skipped_log.append((i, acc["username"], sp.get("name"), item["name"], "stok habis / request gagal"))
                        await asyncio.sleep(1)
                        continue

                    # ambil restoredBps dari response
                    first_result = (result.get("results") or [{}])[0]
                    restore_bps = first_result.get("restoredBps") or first_result.get("needValueBps", 0) - current_bps

                    # update needs dari spiritUpdates
                    for su in result.get("spiritUpdates", []):
                        if su["id"] == sp["id"]:
                            sp_needs = su.get("needs", sp_needs)
                            break
                    current_bps = sp_needs.get(need_field, current_bps)

                    # hitung sisa yang perlu dipake (sudah pake 1x)
                    if restore_bps > 0:
                        remaining_qty = calc_capped_quantity(current_bps, restore_bps, quantity - 1)
                    else:
                        remaining_qty = 0

                    if remaining_qty > 0:
                        await asyncio.sleep(1)
                        result2 = await run_use_step(i, acc["token"], sp["id"], item["slug"], remaining_qty)
                        if result2 is None:
                            skipped_log.append((i, acc["username"], sp.get("name"), item["name"], "stok habis di pemakaian lanjutan"))
                        else:
                            for su in result2.get("spiritUpdates", []):
                                if su["id"] == sp["id"]:
                                    sp_needs = su.get("needs", sp_needs)
                                    break
                    else:
                        print(f"[{i}] {item['name']}: {need_field} udah cukup setelah 1x, stop.")
                else:
                    # fallback kalau kategori gak dikenal
...
async def run_debug_mode():
    ...
    # 3. Raw game-bootstrap (cek ada stok item gak)
    await asyncio.sleep(2)
    print("[DEBUG] Fetching game-bootstrap...")
    status3, resp3 = await game_bootstrap(acc["token"])
    debug_out["game_bootstrap"] = {"status": status3, "response": resp3}
...
async def run_open_daily_chest_mode():
    """Mode 13: claim daily lalu langsung open chest kalau dapet dari bonus streak.
    Kalau udah claimed hari ini tapi belum open, sayang gak bisa — chestInstanceId
    cuma ada di response daily-claim, server gak expose di endpoint lain."""
    tokens = load_tokens(TOKENS_FILE)
    if not tokens:
        print(f"{TOKENS_FILE} kosong. Jalankan mode Register dulu.")
        return
    ...
    selected = select_tokens(tokens)
    print(f"\nClaim daily + open chest buat {len(selected)} akun...\n")

    for i, acc in enumerate(selected, start=1):
        print(f"[{i}] @{acc['username']} (id {acc['telegramId']})")
        d = await run_daily_claim_step(i, acc["token"])
        if not d:
            await asyncio.sleep(1)
            continue

        chest_instance_id = d.get("_chestInstanceId")
        if chest_instance_id:
            await asyncio.sleep(1)
            await run_chest_open_step(i, acc["token"], chest_instance_id)
        else:
            print(f"[{i}] Daily claim sukses tapi gak ada chest di reward hari ini.")
        await asyncio.sleep(1)
...
async def main():
    ...
    print("13. Open daily chest (langsung open tanpa beli)")
    mode = input("Pilih mode [1-13]: ").strip()
    ...
    if mode == "1":
        ...
        await run_open_daily_chest_mode()
    else:
        print("Pilihan tidak valid, keluar.")
        sys.exit(1)
