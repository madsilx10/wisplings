...
async def run_onboarding_step(index, token, action):
...
async def run_care_step(index, token, spirit_id, item_slug):
...
async def process_account(index, session_string):
    ...
    if not onb:
        return
    spirit_id = onb["starterSpirit"]["id"]
    await asyncio.sleep(1)
    ...
    # inventory dari starter kit: espresso(energy), wipe(clean), snack(food)
    for slug in ("espresso", "wipe", "snack"):
