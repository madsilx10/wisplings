...
async def run_use_mode():
    ...
    for i, acc in enumerate(selected, start=1):
        print(f"[{i}] @{acc['username']} (id {acc['telegramId']})")
        if not acc["spiritId"]:
            print(f"[{i}] spiritId gak ada di {TOKENS_FILE}, coba ambil otomatis lewat game-bootstrap...")
            fetched = await fetch_spirit_id(acc["token"])
            if fetched:
                acc["spiritId"] = fetched
                tokens_updated = True
                print(f"[{i}] spiritId ketemu: {fetched}")
            else:
                raw = input(
                    f"[{i}] Gagal auto-fetch spiritId. Masukin manual kalau tau "
                    f"(Enter buat skip akun ini): "
                ).strip()
                if raw:
                    acc["spiritId"] = raw
                    tokens_updated = True
                else:
                    print(f"[{i}] skip, spiritId kosong.")
                    continue
        for item, quantity in tasks:
    ...
    if tokens_updated:
        ...
        print(f"\n{TOKENS_FILE} diupdate dengan spiritId yang baru diisi manual.")
