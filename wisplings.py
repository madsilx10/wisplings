...
async def run_open_daily_chest_mode():
    """Mode 13: open chest yang didapat dari daily (tanpa beli).
    Fetch daily-status buat dapetin chestInstanceId, lalu langsung chest-open."""
    tokens = load_tokens(TOKENS_FILE)
    if not tokens:
        print(f"{TOKENS_FILE} kosong. Jalankan mode Register dulu.")
        return
    ...
    selected = select_tokens(tokens)
    print(f"\nOpen daily chest buat {len(selected)} akun...\n")

    for i, acc in enumerate(selected, start=1):
        print(f"[{i}] @{acc['username']} (id {acc['telegramId']})")
        status, resp = await daily_status(acc["token"])
        if status != 200 or not resp.get("ok"):
            print(f"[{i}] Gagal fetch daily-status ({status}): {resp}")
            await asyncio.sleep(1)
            continue

        data = resp.get("data", {})

        # cari chestInstanceId — bisa di root data atau nested di rewards/chest
        chest_instance_id = (
            data.get("chestInstanceId")
            or (data.get("chest") or {}).get("chestInstanceId")
            or (data.get("reward") or {}).get("chestInstanceId")
        )

        if not chest_instance_id:
            # coba cari di semua key yang ada
            found = None
            for k, v in data.items():
                if isinstance(v, dict) and v.get("chestInstanceId"):
                    found = v["chestInstanceId"]
                    break
            chest_instance_id = found

        if not chest_instance_id:
            print(f"[{i}] Gak ketemu chestInstanceId di daily-status. Raw data: {data}")
            await asyncio.sleep(1)
            continue

        print(f"[{i}] chestInstanceId: {chest_instance_id}, langsung open...")
        await asyncio.sleep(1)
