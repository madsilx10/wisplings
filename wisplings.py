...
async def process_account(index, session_string):
    ...
    if not init_data:
        print(f"[{index}] GAGAL TOTAL ambil initData, akun ini di-skip (gak masuk {TOKENS_FILE}).")
        return False
    ...
    if status not in (200, 201) or not auth_resp.get("ok"):
        print(f"[{index}] GAGAL TOTAL auth-telegram, akun ini di-skip (gak masuk {TOKENS_FILE}).")
        return False
...
async def main():
    ...
    mode = input("Pilih mode [1-5]: ").strip()
