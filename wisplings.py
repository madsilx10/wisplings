...
# Sisanya (double_shot, energy_drink, full_charge, meal, feast, banquet,
# sponge, bath, royal_spa) masih tebakan dari nama tampilan -> slug
# (lowercase, spasi jadi underscore). Kalau ternyata beda pas dicoba,
# tinggal betulin slug di sini aja, gak usah ubah logic lain.
ITEM_CATALOG = {
    "energy": [
        {"slug": "espresso", "name": "Espresso", "price": 20},
        {"slug": "double_shot", "name": "Double Shot", "price": 60},
        {"slug": "energy_drink", "name": "Energy Drink", "price": 180},
        {"slug": "full_charge", "name": "Full Charge", "price": 900},
    ],
    "food": [
    ...
    "wash": [
        ...
        {"slug": "royal_spa", "name": "Royal Spa", "price": 900},
    ],
}
# =================================================
...
def select_indexed(items, label):
...
async def get_init_data(session_string):
    ...
    async with app:
        ...
        try:
            ...
            print(f"    debug: /start terkirim dengan start_param={START_PARAM}")
...
async def run_use_step(index, token, spirit_id, item_slug, quantity, retries=3):
    for attempt in range(1, retries + 1):
        ...
        if status in (200, 201) and resp.get("ok"):
            d = resp["data"]
            print(f"[{index}] use {item_slug} x{quantity} sukses: {d}")
            return d
        if status == 500 and attempt < retries:
            wait = 2 * attempt
...
async def run_buy_mode():
    ...
    selected = select_indexed(tokens, TOKENS_FILE)
    print(f"\nBeli {item['name']} (slug: {item['slug']}) x{quantity} buat {len(selected)} akun...\n")
...
async def run_use_mode():
    ...
    quantity = ask_quantity()
    selected = select_indexed(tokens, TOKENS_FILE)
    print(f"\nPakai {item['name']} (slug: {item['slug']}) x{quantity} buat {len(selected)} akun...\n")

    for i, acc in enumerate(selected, start=1):
        print(f"[{i}] @{acc['username']} (id {acc['telegramId']})")
        if not acc["spiritId"]:
            print(f"[{i}] spiritId gak ada di {TOKENS_FILE} (akun ini kemungkinan register-nya pakai skrip lama). "
                  f"Skip. Isi manual kolom ke-4 di {TOKENS_FILE} kalau tau spiritId-nya, atau register ulang.")
            continue
        await run_use_step(i, acc["token"], acc["spiritId"], item["slug"], quantity)
        await asyncio.sleep(1)
...
async def main():
    ...
    mode = input("Pilih mode [1-4]: ").strip()
