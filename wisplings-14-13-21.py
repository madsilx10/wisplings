...
async def run_buy_mode():
    ...
    for i, acc in enumerate(selected, start=1):
        ...
        # fetch stok saat ini dari game-bootstrap
        owned_map = {}
        if stock_limit:
            gb_status, gb_resp = await game_bootstrap(acc["token"])
            if gb_status == 200 and gb_resp.get("ok"):
                inv = gb_resp["data"].get("home", {}).get("inventory", [])
                owned_map = {it["slug"]: it["quantity"] for it in inv}
            await asyncio.sleep(1)

        for item, quantity in tasks:
            if stock_limit:
                owned = owned_map.get(item["slug"], 0)
                if owned >= stock_limit:
                    print(f"[{i}] {item['name']}: stok udah {owned}, limit {stock_limit} — skip.")
                    continue
                actual_qty = min(quantity, stock_limit - owned)
                if actual_qty < quantity:
                    print(f"[{i}] {item['name']}: stok {owned}, beli {actual_qty}x (dari {quantity}x) biar pas limit {stock_limit}.")
                await run_buy_step(i, acc["token"], item["slug"], actual_qty)
            else:
...
async def run_use_mode():
    ...
    for i, acc in enumerate(selected, start=1):
        ...
        # fetch restoreBps per item dari game-bootstrap
        restore_map = {}
        gb_status, gb_resp = await game_bootstrap(acc["token"])
        if gb_status == 200 and gb_resp.get("ok"):
            inv = gb_resp["data"].get("home", {}).get("inventory", [])
            restore_map = {it["slug"]: it["restoreBps"] for it in inv if it.get("restoreBps")}
        await asyncio.sleep(1)
        ...
        for j, sp in enumerate(spirit_targets):
            ...
            for item, quantity in tasks:
                ...
                restore_bps = restore_map.get(item["slug"])
                ...
                if need_field and restore_bps:
                    ...
                    if actual_qty == 0:
                        print(f"[{i}] skip {item['name']} — nambah 1x bakal overflow 100%.")
                        continue
                    if actual_qty < quantity:
                        print(f"[{i}] {item['name']}: pake {actual_qty}x (dari {quantity}x) biar pas 100%.")
                    ...
                    # fallback kalau restoreBps gak ada di bootstrap
